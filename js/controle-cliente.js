...
btnAdicionar.addEventListener('click', () =>{
    modoEdicao = false;
    tituloModal.textContent = "Adicionando jogadores"
    limparModalCliente();
    modalCliente.show();
});
...
btnSalvar.addEventListener('click', () => {
    ...
    if(!cliente.cpfOuCnpj || !cliente.email || !cliente.email || !cliente.telefone || !cliente.nome ){
    
       Swal.fire('Todos os campos são obrigatorios.')
    ...
    Swal.fire ("Jogador Cadastraso com Sucesso");
...
function editarCliente(id){
    modoEdicao = true;
    tituloModal.textContent = "Editando jogadores"
    ...
    modalCliente.show();
    //Swal.fire ("Jogador editado com Sucesso");
...
function excluirCliente(id){
    ...
    if(confirm("Deseja realmente excluir o Jogador " + cliente.nome)){
...
function excluirClienteBackEnd(cliente){
    ...
    .then(() => {
        ...
        Swal.fire({
            position: 'top-end',
            icon: 'success',
            title: 'Jogador excluido com sucesso!',
            showConfirmButton: false,
            timer: 2500
        });
    })
